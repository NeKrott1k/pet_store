<div class="container">
    <div class="page-header">
        <div class="promotions-header">
            ...
            <span class="promotions-count"><?= count($products) ?> <?= count($products) === 1 ? 'товар' : (count($products) < 5 ? 'товара' : 'товаров') ?></span>
        </div>
    </div>

    <?php if (empty($products)): ?>
        <div class="catalog-empty"><p>Акций пока нет. Следите за обновлениями!</p></div>
    <?php else: ?>
        <div class="cards-grid">
            <?php foreach ($products as $p):
                $price = $p['price'] * (1 - $p['discount_percent'] / 100);
            ...
            <div class="card">
                <a href="/product/<?= $p['id'] ?>">
                    <div class="card__img-wrap">
                        ...
                        <div class="card__badges"><span class="badge badge--sale">Скидка <?= $p['discount_percent'] ?>%</span></div>
                    </div>
                </a>
                <div class="card__body">
                    ...
                    <div class="card__footer">
                        <div>
                            <span class="card__price card__price-new"><?= number_format($price, 0, '.', ' ') ?> ₽</span>
...
async function AddToCart(btn) {
    ...
    try {
        ...
        if (data.success) {
            ...
            btn.parentElement.innerHTML = `<div class="qty-control" data-product-id="${pid}"><button class="qty-control__btn" onclick="changeQty(this,-1)">−</button><span class="qty-control__value">1</span><button class="qty-control__btn" onclick="changeQty(this,1)">+</button></div>`;
        }
